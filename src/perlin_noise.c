#include <math.h>
#include <stdlib.h>

static int p[512]; // Double the permutation array size for simplification

// The original permutation array
static const int permutation[] = { 151,160,137,91,90,15,
    131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,8,99,37,240,21,10,23,
    190, 6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,35,11,32,57,177,33,
    88,237,149,56,87,174,20,125,136,171,168, 68,175,74,165,71,134,139,48,27,166,
    77,146,158,231,83,111,229,122,60,211,133,230,220,105,92,41,55,46,245,40,244,
    102,143,54, 65,25,63,161, 1,216,80,73,209,76,132,187,208, 89,18,169,200,196,
    135,130,116,188,159,86,164,100,109,198,173,186, 3,64,52,217,226,250,124,123,
    5,202,38,147,118,126,255,82,85,212,207,206,59,227,47,16,58,17,182,189,28,42,
    223,183,170,213,119,248,152, 2,44,154,163, 70,221,153,101,155,167, 43,172,9,
    129,22,39,253, 19,98,108,110,79,113,224,232,178,185, 112,104,218,246,97,228,
    251,34,242,193,238,210,144,12,191,179,162,241, 81,51,145,235,249,14,239,107,
    49,192,214, 31,181,199,106,157,184, 84,204,176,115,121,50,45,127, 4,150,254,
    138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180
};


void initPerlinNoise(unsigned int seed) {
    // Initialize the permutation vector with the reference permutation
    for (int i = 0; i < 256; i++) {
        p[i] = permutation[i];
    }

    // Seed the random generator
    srand(seed);

    // Shuffle the permutation vector
    for (int i = 0; i < 256; i++) {
        int j = rand() & 255;
        // Swap p[i] and p[j]
        int temp = p[i];
        p[i] = p[j];
        p[j] = temp;
    }

    // Duplicate the permutation to avoid overflow
    for (int i = 0; i < 256; i++) {
        p[256 + i] = p[i];
    }
}

float fade(float t) {
    // Fade function as defined by Ken Perlin
    // 6t^5 - 15t^4 + 10t^3
    return t * t * t * (t * (t * 6 - 15) + 10);
}

float lerp(float a, float b, float t) {
    // Linear interpolate between a and b
    return a + t * (b - a);
}

float grad(int hash, float x, float y) {
    int h = hash & 15; // CONVERT LO 4 BITS OF HASH CODE
    float u = h < 8 ? x : y; // INTO 12 GRADIENT DIRECTIONS.
    float v = h < 4 ? y : h == 12 || h == 14 ? x : 0;
    return ((h & 1) == 0 ? u : -u) + ((h & 2) == 0 ? v : -v);
}

float perlin(float x, float y) {
    int xi = (int)floor(x) & 255;
    int yi = (int)floor(y) & 255;
    float xf = x - floor(x);
    float yf = y - floor(y);
    float u = fade(xf);
    float v = fade(yf);

    int aaa = p[p[xi] + yi];
    int aba = p[p[xi] + yi + 1];
    int aab = p[p[xi + 1] + yi];
    int abb = p[p[xi + 1] + yi + 1];

    float x1, x2, y1;//y2
    x1 = lerp(grad(aaa, xf, yf), grad(aab, xf - 1, yf), u);
    x2 = lerp(grad(aba, xf, yf - 1), grad(abb, xf - 1, yf - 1), u);
    y1 = lerp(x1, x2, v);

    return (y1 + 1) / 2; // Normalize to [0, 1]
}
